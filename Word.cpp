...
JunYe_Word::JunYe_Word(){
	...
	Wd.App.OlePropertySet("DisplayAlerts", wdAlertsNone);
	Wd.App.OlePropertySet("Visible", false);
...
bool  __fastcall JunYe_Word::getBookmark(String mark){
	int retry = 0;
	do{
		try{
			Wd.Range = Wd.ActiveDoc.OlePropertyGet("Bookmarks");
			bool bobo = Wd.Range.OleFunction("Exists", OleStr(mark));
			return bobo;
		}catch(EOleSysError *e){
			if (e->ErrorCode == -2147418111){
				retry++;
				BugReport(L"getBookmark Fail, retry for "+IntToStr(retry)+" times");
				Timer(200);
			}else{
				BugReport("Error", "Word::getBookmark(String mark)");
				throw;
			}
...
bool __fastcall JunYe_Word::gotoBookmark(String mark){
	int retry = 0;
	do{
		try{
			...
			if (e->ErrorCode == -2147418111){
				retry++;
				BugReport(L"gotoBookmark Fail, retry for "+IntToStr(retry)+" times");
				Timer(200);
			}else{
				BugReport("Error", "Word::gotoBookmark(String mark)");
				throw;
			}
		}catch(Exception *e){
			BugReport("Error", "Word::gotoBookmark(String mark)");
...
bool __fastcall JunYe_Word::paste_2(){
	try{
		Wd.Range.OleFunction("Paste");
		return true;
	}catch(EOleSysError *e){
		BugReport(L"Word::paste_2 fail, "+ e->Message);
	}
	return false;
}
